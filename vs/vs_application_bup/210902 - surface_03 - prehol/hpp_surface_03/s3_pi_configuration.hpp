...
// -------- TARGET ------------------------------------------------
	...
	// ---- ruler
		VOID Surface_03::SetTargetLoiRulerVisible(BOOL target_ruler_visible)
		{
			...
			pi_surface_03->SetObjectVisible(PI_Surface_03::GOB_EXP_LOI_SECT_IDX, target_ruler_visible);
	...
	// ---- translate
		VOID Surface_03::SetTargetLoiTranslate(BOOL target_transform)
		{
			pi_surface_03->_configuration.target_transform = target_transform;
		}
		BOOL Surface_03::GetTargetLoiTranslate()
		{
	...
	// --- substantive get & set
		...
		FLOAT Surface_03::GetTargetLoiLocationHeight(UINT index)
		{
			...
			// ---- so, assuming legal x & z ordinates
				...
				INT row = (INT)(z_ok * (FLOAT)hf_width);
					row = hf_width - row;
				INT col = (INT)(x_ok * (FLOAT)hf_width);

			// ---- Y!
				UINT new_extent = row * stride + col;
				new_extent = new_extent >= extent ? extent - 1 : new_extent;
				FLOAT height = *(array + new_extent) * GetTargetLoiHeightScalar();
			...
			)
		{
			...
			// ---- sample
				...
				for (UINT sample=0; sample<256; sample++)
				{
					...
					// ---- interpolate
						DOUBLE rmu = (FLOAT)sample / (FLOAT)num_samples;
						FLOAT x = (FLOAT)( (1 - rmu)*x1 + rmu * x2 );
						FLOAT z = (FLOAT)( (1 - rmu)*z1 + rmu * z2 );
					...
					// ---- Y!
						UINT new_extent = row * stride + col;
						new_extent = new_extent > extent ? extent - 1 : (new_extent < 0 ? 0 : new_extent);
						*(y_samples+sample) = *(array + new_extent) * GetTargetLoiHeightScalar();
				}
		...
		VOID Surface_03::SetTargetLoiIncr(FLOAT x, FLOAT y, FLOAT z)
		{
