			...
			default:
				...
				if ( pi_surface_03->runtime )
				{
					...
					// ---- setup proportional to size of surface
						FLOAT dist = pi_surface_03->_configuration.acquire_dimensions.size * 2.0f;
					...
					// ---- increment
						kamera->SetPositionIncr(kpv/4);    // axis vector mode
						kamera->SetPanTiltIncr(kpv/8);
						kamera->SetTranslateIncr(kpv / 4);
						kamera->SetRotateIncr(kpv/8);
...
INT Surface_03::SetupExploreMaterials(UINT index, Surface_03 *p_s_03)
{
	...
	// ---- light colour as per _configuration
		VsColour light_colour = {
			pimp->_configuration.explore_light[index].diffuse.red   * light_scalar,
			pimp->_configuration.explore_light[index].diffuse.green * light_scalar,
			pimp->_configuration.explore_light[index].diffuse.blue  * light_scalar,
			0
		};
