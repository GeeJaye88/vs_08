...
INT Model::SetupGraphics()
{
	...
	// ---- init MEASURE by storing current kamera & target param
		TintBodyUpdateKamera(TINT_BODY_MEASURE_STORE);
...
INT Model::Display(vs_system::PanelContext *panel_gfx_context, CHAR *fred)
{
	...
	// ---- measure ?
		BOOL measure = surface_03->GetTargetLoiMeasure();
	...
	// ---- set fov
		...
		if ( measure )
		{
			result = Tint_Measure_UpdateModel((INT)this, 0);
			FLOAT v_fov = surface_03->GetTargetLoiMeasureFov();
			perspective[0] = v_fov;    // v fov
			perspective[1] = 0.1f;     // near
			perspective[2] = 40000.0f; // far
		}
		else
		{
			perspective[0] = 40.0f;   // vertical fov
			perspective[1] = 0.1f;    // near
