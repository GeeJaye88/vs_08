...
INT Model::SetupGraphics()
{
	...
	// ---- init MEASURE
		TintBodyUpdateKamera(TINT_BODY_MEASURE_STORE_KAMERA);
...
INT Model::Display(vs_system::PanelContext *panel_gfx_context, CHAR *fred)
{
	...
	// ---- target measure
		BOOL  measure     = pi_model->surface_03->GetTargetLoiMeasure();
		FLOAT measure_fov = pi_model->surface_03->GetTargetLoiMeasureFov();


	// ---- set fov & clipping
		FLOAT *perspective = panel_gfx_context->GetPerspective();
		perspective[0] =   measure ? measure_fov : 40;   // verticle fov
		perspective[1] =     0.1f;                       // near clip
		perspective[2] =   measure ? 40000.0f : 1000.0f; // far clip
