	...
	// ---- elem
		for (int i = 0; i < PI_Surface_03::MAX_EXP_LOI_; i++)
		{
			...
			// ---- param
				...
				param[vs_system::ObjConfig::BOX_FACE_MODE]    = 0; // measure mode
...
INT Surface_03::ObjectLoiSect_Display(INT gfx_object_index, Surface_03 *p_s_03, BOOL visible)
{
	...
	// ---- local
		PI_Surface_03 *pimp = p_s_03->pi_surface_03;
		UINT target_loi_index = p_s_03->GetTargetLoiIndex();
		if ( target_loi_index == 999 ) return VS_ERROR;
	...
	// ---- elem
		for (int i = 0; i < PI_Surface_03::MAX_EXP_LOI_; i++)
		{
			...
			// ---- visible ?
				BOOL elm_visible = (i == target_loi_index ? TRUE : FALSE) && visible;
				...
				if ( elm_visible )
				{
					...
					//	char msg[128];
					//	sprintf_s(msg, 128, "display %d %d\n", i, target_loi_index);
					//	OutputDebugString(msg);

					//// ---- init transform matrix
					//	BOOL  target_measure = p_s_03->GetTargetLoiMeasure();
					//	FLOAT target_rotation = p_s_03->GetTargetLoiRotation(target_loi_index);
					//	VsVector3 *v = p_s_03->GetTargetLoi(target_loi_index);
					//	if ( target_measure )
					//	{
					//		FLOAT yoff = -10.0;
					//		FLOAT scale = 0.2f;
					//		pimp->matrix_stack->Scale(scale, scale, scale);
					//		pimp->matrix_stack->Rotate(-target_rotation, 'Y');
...
INT Surface_03::ObjectLoiSect_Update(Surface_03 *p_s_03)
{
	...
	// ---- local
		...
		if ( target_index == 999 ) return VS_ERROR;
	...
	// ---- store parameters
		...
		// ---- measure mode
			//INT target_measure = (INT)p_s_03->GetTargetLoiMeasure();
			//param[vs_system::ObjConfig::BOX_FACE_MODE] = (FLOAT)target_measure; // measure flag
			//if ( target_measure )
			//{
			//	float fred = 2;
			//}
		...
		// ---- at
			...
			param[vs_system::ObjConfig::BOX_FLAG_2] = y_ext + y_hgt; // {was  y_ext) y_top 
			param[vs_system::ObjConfig::BOX_FLAG_3] = y_ext;         // (was -y_hgt) y_bot
