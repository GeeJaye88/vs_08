...
// ---------- s2_model_project_framework.hpp ----------
/*!
\file s2_model_project_framework.hpp
...
INT Model::ProjectSetup()
{
	...
	// ---- allocate, construct, setup and configure
		pi_model->surface_02 = new model::Surface_02();
		model::Surface_02 *surface_02 = pi_model->surface_02;
		result = surface_02->Setup(pi_model->app_dependency);

	// ---- set object config
		result = surface_02->SetObjConfig(pi_model->obj_config);
		result = surface_02->SetImgProps(pi_model->img_props);
...
INT Model::ProjectSetupGraphics()
{
	...
	// ---- lights, kamera, action....
		result = surface_02->SetupGraphics(pi_model->app_kamera[0]);
		result = surface_02->SetVisibleObjects(TINT_BODY_ACQUIRE_LID, surface_02);
	...
	// ---- only one scene
		INT handle_scene = 0;
		result = surface_02->GetDisplayListHandle(&handle_scene, "Scene");
		if ( handle_scene != 0 )
		{
	...
	// ---- runtime ?
		if ( pi_model->runtime )
		{
			...
			// ---- initialiase acquisition
				surface_02->SetupAcquireParameters();
				surface_02->SetupAcquire(s_pi_model->app_kamera[0]);

			// ---- setup acquisition ( s2_tint_1_callback_acquire.hpp )
				result = Acquire_SetupBuffers();

			// ---- initialise acquire tint model & select source surface
		...
		{
			// ---- update selected tint body
				TintBodyUpdate(TINT_BODY_SETUP_GFX_LID); //pi_model->tint_setng->GetSelectedBodyId());
		}

	// ---- set visible objects per selected body
		result = surface_02->SetVisibleObjects(
				pi_model->tint_setng->GetSelectedBodyId(),
				surface_02
			);
...
INT Model::ProjectSetupDisplay()
{
				...
				if ( target_id >= 0 && target_id <= 9 )
				{
					...
					// ---- new target ?
						if ( new_target )
						{
							...
							pi_model->surface_02->SetTargetLoiIndex(target_id);
						}

					// ---- target loi
						surface_02->SetTargetLoiIndex(target_id);
						VsVector3 *v = surface_02->GetTargetLoi();
	...
	// ---- setup
		result = surface_02->SetupDisplay(pi_model->app_kamera[0]);
...
INT Model::ProjectCleanupGraphics()
{
	...
	// ---- lights, kamera, action....
		result = surface_02->CleanupGraphics();
