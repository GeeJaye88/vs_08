...
// ---------- s2_tint_3_callback_image.hpp ----------
/*!
\file s2_tint_3_callback_image.hpp
\brief
	...
	)
{
	...
	// ---- lights setup
		...
		delete[] param_values;
		s_pi_model->surface_02->SetExploreLightsEnabled(lights_enabled);
	...
	)
{
	...
	// ---- get lights setup
		BOOL lights_enabled[5];
		surface_02->GetExploreLightsEnabled(lights_enabled);
					...
					// ---- by group id (e.g. 'L')
							...
							case 'P':
								{
									...
									Kpad_Get(handle_model, GetParamGroup());
						...
						// ---- colour
							case 'M' :
								{
									// ---- flip colourlighting
										BOOL colour_enabled = surface_02->GetExploreLightColouredEnabled();
										colour_enabled = colour_enabled ? FALSE : TRUE;
										surface_02->SetExploreLightColouredEnabled(colour_enabled);

									// ---- update explore lighting rig
										result = surface_02->ObjectExpLgtAxis_UpdateAll(surface_02);
						...
						// ---- specular
							case 'N' :
								{
									BOOL specular_enabled = surface_02->GetExploreLightSpecularEnabled();
									specular_enabled = specular_enabled ? FALSE : TRUE;
									surface_02->SetExploreLightSpecularEnabled(specular_enabled);
								}
								break;
	...
	cleanup:
		...
		// ---- set lighting
			surface_02->SetExploreLightsEnabled(lights_enabled);
			surface_02->SetupExploreLighting(1);
			surface_02->SetupExploreMaterials(1);
													...
													// ---- copy parameter to all - IF NOT ambient light
														if ( tint_light_param.copy_param_to_all && ! tint_light_param.ambient_light_on )
														{
															...
															// ---- reset lights and surface material
																model::Surface_02 *surface_02 = s_pi_model->surface_02;
																result = surface_02->SetupExploreLighting(1);
																result = surface_02->SetupExploreMaterials(1);
