...
*/
INT Surface_02::ObjectLoiAxis_Setup(INT gfx_object_index, Surface_02 *p_s_02)
{
	...
	// ---- elem
		for (int i = 0; i < PI_Surface_02::MAX_EXP_LOI_; i++)
		{
			...
			// ---- create ? - or reconfig
				if ( pimp->runtime )
				{
					pimp->gfx_elem_handle[PI_Surface_02::ELM_EXP_LOI_AXIS_LIST_IDX][i] = gfx->AddElement(
				...
				else
				{
					pimp->gfx->SetElementConfigureBitmask(
							pimp->gfx_elem_handle[PI_Surface_02::ELM_EXP_LOI_AXIS_LIST_IDX][i],
...
\return INT - ERROR <=0 < VS_OK
*/
INT Surface_02::ObjectLoiAxis_Display(INT gfx_object_index, Surface_02 *p_s_02, BOOL visible)
{
			...
			// ---- visible ?
				...
				if ( elm_visible )
				{
					...
					// ---- transform
						...
						FLOAT angle = p_s_02->GetTargetLoiRotation(i);
						pimp->matrix_stack->Translate(v->x, 0, v->z);
...
\return INT - ERROR <=0 < VS_OK
*/
INT Surface_02::ObjectLoiAxis_HideAll(Surface_02 *p_s_02)
{
...
\return INT - ERROR <=0 < VS_OK
*/
INT Surface_02::ObjectLoiAxis_Reset(Surface_02 *p_s_02)
{
	// ---- local
		PI_Surface_02 *pimp = p_s_02->pi_surface_02;
...
\return INT - ERROR <=0 < VS_OK
*/
INT Surface_02::ObjectLoiAxis_Update(Surface_02 *p_s_02)
{
	...
	// ---- reconfigure
		INT hr = pimp->gfx->SetElementConfigureBitmask(
				pimp->gfx_elem_handle[PI_Surface_02::ELM_EXP_LOI_AXIS_LIST_IDX][target_index],
				VS_ELM_SETUP_GEOMETRY
			);
