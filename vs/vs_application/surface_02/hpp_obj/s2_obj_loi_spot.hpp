...
*/
INT Surface_02::ObjectLoiSpot_Setup(INT gfx_object_index, Surface_02 *p_s_02)
{
	...
	// ---- elem
		for (int i = 0; i < PI_Surface_02::MAX_EXP_LOI_; i++)
		{
			...
			// ---- create ? - or reconfig
				if ( pimp->runtime )
				{
					pimp->gfx_elem_handle[PI_Surface_02::ELM_EXP_LOI_SPOT_LIST_IDX][i] = gfx->AddElement(
				...
				else
				{
					pimp->gfx->SetElementConfigureBitmask(
							pimp->gfx_elem_handle[PI_Surface_02::ELM_EXP_LOI_SPOT_LIST_IDX][i],
...
\return INT - ERROR <=0 < VS_OK
*/
INT Surface_02::ObjectLoiSpot_Display(
			...
			// ---- visible ?
				...
				if ( visible )
				{
					...
					// ---- Y offset - only LOI sphere gets moved by target height!
						FLOAT y_off = p_s_02->CalculateTargetLoi_Surface_Y(i);
...
\return INT - ERROR <=0 < VS_OK
*/
INT Surface_02::ObjectLoiSpot_HideAll(Surface_02 *p_s_02)
{
...
\return INT - ERROR <=0 < VS_OK
*/
INT Surface_02::ObjectLoiSpot_Reset(Surface_02 *p_s_02)
{
...
\return INT - ERROR <=0 < VS_OK
*/
INT Surface_02::ObjectLoiSpot_Update(Surface_02 *p_s_02)
{
	return VS_OK;
}
