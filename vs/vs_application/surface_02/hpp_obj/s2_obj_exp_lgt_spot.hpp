...
// ---------- s2_obj_exp_lgt_spot.hpp ----------
/*!
\file s2_obj_exp_lgt_spot.h
\brief 
...
*/
INT Surface_02::ObjectExpSpot_Setup(INT index, Surface_02 *p_s_02)
{
	...
	// ---- local
		...
		// ---- create or (re)config sphere
			if (pimp->runtime )
			{
				pimp->gfx_elem_handle[PI_Surface_02::ELM_EXP_LGT_SPOT_LIST_IDX][index] =
					pimp->gfx->AddElement(
...
\return INT - ERROR <=0 < VS_OK
*/
INT Surface_02::ObjectExpSpot_Display(INT index, Surface_02 *p_s_02, BOOL visible)
{
	...
	// ---- transform PHI
		...
		//		pimp->_configuration.acquire_light[index].position.x,
		//		pimp->_configuration.acquire_light[index].position.y,
		//		pimp->_configuration.acquire_light[index].position.z
		//	);
