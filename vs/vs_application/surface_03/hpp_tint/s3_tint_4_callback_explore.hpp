...
public:
	...
	// ---- item element logical id descriptors
		enum
		{
			...
			ELEMENT_LID_TARGET_PROFILE = 'c',
			ELEMENT_LID_TARGET_TFORM   = '(',
			ELEMENT_LID_TARGET_TSECT   = ')',
		...
		CHAR *update_text[6] = 
			{
				...
				" Object X, Z & Y Translation",
				" Object Y, X & Z Rotation",
				" Camera & Object"
			};

		CHAR *control_text[8][2] =
			{
				{ "Z & X Axis       Y Axis",   "" },
				{ "Fly & Strafe     Climb",    "" },
				{ "Zoom & Pan    Tilt",        "" },
	...
	// ---- build
		{
			...
			// ---- select camera mode
				...
				TINT_SET_TARC(Tint::ITEM_RADIO_LOCAL,    4, 5, 3)          // world
				TINT_ADD_ICON()
				...
				TINT_ADD_ICON()
				TINT_SET_TARCX(Tint::ITEM_RADIO_LOCAL,   4, 5, 6, 4.1f)   // object
				TINT_ADD_ICON()
				TINT_SET_TARCXLI(Tint::ITEM_RADIO_LOCAL, 4, 5, 7, 5.2f, TRUE, lincr)
			...
			// ---- targets
				...
				// ---- row 1
					...
					TINT_SET_TARCX(Tint::ITEM_SWITCH,        5, 0, 4, 0.0f)   // spot
					TINT_ADD_ICON()
					TINT_SET_TARCX(Tint::ITEM_SWITCH,        5, 0, 5, 1.4f)   // rule
					TINT_ADD_ICON()
					TINT_SET_TARCX(Tint::ITEM_SWITCH,        5, 0, 6, 2.8f)   // profile rule
					TINT_ADD_ICON()
					TINT_SET_TARCX(Tint::ITEM_SWITCH,        5, 0, 7, 3.9f)   // profile
					TINT_ADD_ICON()
					TINT_SET_TARCX(Tint::ITEM_SWITCH,        5, 0, 7, 5.3f)   // update XZ & profile rotation
					TINT_ADD_ICON()
					TINT_SET_TARCXLI(Tint::ITEM_SWITCH,      5, 0, 7, 6.4f, TRUE, lincr / 4) // profile slide
		...
		// ---- get set indicees
			UINT mesh_display_index      = 0;
			UINT mesh_density_index      = 0;
			UINT mesh_move_index         = 0;
			UINT mesh_target_first_index = 0;
			UINT mesh_target_last_index  = 0;
			UINT mesh_speed_index        = 0;
			for (INT e = 0; e < noi; e++)
			{
				CHAR lid_char = (UINT)tint_explore_param.element_item_lid_char[e];
				switch ( lid_char )
				{
					case tint_explore_param.ELEMENT_LID_SOLID       : mesh_display_index      = e; break;
					case tint_explore_param.ELEMENT_LID_LOW         : mesh_density_index      = e; break;
					case tint_explore_param.ELEMENT_LID_UPDATE_FIRST: mesh_move_index         = e; break;
					case tint_explore_param.ELEMENT_LID_TARGET_LOI  : mesh_target_first_index = e; break;
					case tint_explore_param.ELEMENT_LID_TARGET_LAST : mesh_target_last_index  = e; break;
					case tint_explore_param.ELEMENT_LID_SPEED       : mesh_speed_index        = e; break;
					default: break;
				}
			}
		...
		// ---- set surface solid & textured
			{
				...
				li[mesh_display_index + 0].SetStatus(solid);

				BOOL textured = surface_03->GetMeshTextured();
				surface_03->SetMeshTextured(textured);
				li[mesh_display_index + 1].SetStatus(textured);
			}
		...
		// ---- set mesh densisty mode
			{
				...
				switch ( mdm )
				{
					...
					case surface_03->HIGH:
						li[mesh_density_index + mdm].SetStatus(TRUE);
						break;
					default:
						{
							mdm = surface_03->LOW;
		...
		// ---- set kamera mode
			{
				...
				switch ( ccm )
				{
					...
					case AppKamera::Mode::ROTATE    :
						li[mesh_move_index + ccm].SetStatus(TRUE);
						break;
					...
					case AppKamera::Mode::TARGET:
						{
							UINT target_index =
								s_pi_model->GetTargetObjectId() +
									mesh_target_first_index;
							if (
									target_index >= mesh_target_first_index &&
									target_index <= mesh_target_last_index
								)
							{
		...
		// ---- set speed
			...
			li[mesh_speed_index].SetStatus(kamera->GetScalarMode());
				...
				switch ( group ) 
				{
								...
								case Tint_Explore_Param::ELEMENT_LID_SOLID:
									{
										...
										surface_03->SetMeshSolid(solid);
										surface_03->UpdateHeightfieldObjects((INT)'E', surface_03, FALSE);
									}
					...
					case Tint_Explore_Param::GROUP_LID_CAMERA_UPDATE:
						{
							...
							// ---- don't zap loi state
								...
								//surface_03->SetTargetLoiTargetModeFALSE);
							...
							// ---- hide -+- fix loi stuff -+-
								...
								surface_03->ObjectLoiAnno_HideAll(surface_03);
					...
					case Tint_Explore_Param::GROUP_LID_CAMERA_LOI:
					{
						switch (element_id)
						{
							...
							case Tint_Explore_Param::ELEMENT_LID_TARGET_PROFILE:
								{
									BOOL profile_visible = surface_03->GetTargetLoiProfileVisible();
									profile_visible = profile_visible ? FALSE : TRUE;
									surface_03->SetTargetLoiProfileVisible(profile_visible);
								}
								break;
							...
							case Tint_Explore_Param::ELEMENT_LID_TARGET_TFORM:
								{
									...
									target_mode = target_mode ? FALSE : TRUE;
									surface_03->SetTargetLoiTargetMode(target_mode);
								}
								break;

							case Tint_Explore_Param::ELEMENT_LID_TARGET_TSECT:
								{
									BOOL section_mode = surface_03->GetTargetLoiSectionMode();
									section_mode = section_mode ? FALSE : TRUE;
									surface_03->SetTargetLoiSectionMode(section_mode);
								;
								}
								break;
					...
					case Tint_Explore_Param::GROUP_LID_CAMERA_TARGET:
						{
							...
							// ---- select the measure loi button
								model->Tint_Measure_Switch(handle_model,
										'F',
										0,
										FALSE
									);
							...
							// ---- set target #
								...
								//s_pi_model->SetTargetObjectIdPrev(1);
								s_pi_model->SetTargetObjectId(tid);
								surface_03->SetTargetLoiIndex(tid);


							// ---- force re-init
								s_pi_model->surface_03->SetTargetLoiInit(FALSE);
							...
							// --- set 6 loi buttons & chrome
								...
								BOOL target_section_visible = surface_03->GetTargetLoiSectionVisible();
								surface_03->SetTargetLoiSectionVisible(target_section_visible);
								model->Tint_Explore_Switch(handle_model,
									Tint_Explore_Param::GROUP_LID_CAMERA_LOI,
									Tint_Explore_Param::ELEMENT_LID_TARGET_SECTION,
									target_section_visible
								);

								BOOL target_profile_visible = surface_03->GetTargetLoiProfileVisible();
								surface_03->SetTargetLoiProfileVisible(target_profile_visible);
								model->Tint_Explore_Switch(handle_model,
										Tint_Explore_Param::GROUP_LID_CAMERA_LOI,
										Tint_Explore_Param::ELEMENT_LID_TARGET_PROFILE,
										target_profile_visible
									);
								...
								model->Tint_Explore_Switch(handle_model,
										Tint_Explore_Param::GROUP_LID_CAMERA_LOI,
										Tint_Explore_Param::ELEMENT_LID_TARGET_TSECT,
										section_mode
									);
							...
							// ---- update loi annotation
								surface_03->ObjectLoiAnno_Update(surface_03);
					...
					case Tint_Explore_Param::GROUP_LID_CAMERA_CONTROL:
						{
							...
							// ---- camera ( ! get delta ! )
								vs_system::AppKamera *kamera = s_pi_model->app_kamera[s_pi_model->app_kamera_index];
								FLOAT delta = kamera->GetScalarMode() ? 250.0f : 50.0f;
	...
	)
{
	...
	// ---- store tint body logical id
		UINT body_id = tint->GetSelectedBodyId();
	...
	// ---- process
		...
		// ---- reset tint body logical id
			result = tint->SetSelectedBody(body_id);
						...
						FLOAT v[] =
							{
								...
								(FLOAT)tint_explore_param.ELEMENT_LID_TARGET_PROFILE,      animate_action,
								(FLOAT)tint_explore_param.ELEMENT_LID_TARGET_TFORM,        animate_action,
								(FLOAT)tint_explore_param.ELEMENT_LID_TARGET_TSECT,        animate_action,
			...
			case Tint_Explore_Param::ELEMENT_LID_TARGET_LOI:
				{
					...
					if ( ccm == vs_system::AppKamera::Mode::TARGET )
					{
						for (INT e = 0; e < tint_explore_param.element_dspl_count; e++)
						{
							...
							if ( lid_char == tint_explore_param.ELEMENT_LID_CONTROL_TEXT )
							{
								BOOL target_mode = surface_03->GetTargetLoiTargetMode();
								if ( target_mode )
								{
									BOOL section_mode = surface_03->GetTargetLoiSectionMode();
									if ( section_mode ) // "Z & X axis        Profile"
									{
										tint_explore_param.element_dspl_info[e].context->SetText(
											tint_explore_param.control_text[7][0]);
									}
									else // "Z & X axis        Rotate"
									{
										tint_explore_param.element_dspl_info[e].context->SetText(
											tint_explore_param.control_text[6][0]);
								...
								{
									tint_explore_param.element_dspl_info[e].context->SetText(
										tint_explore_param.control_text[5][0]);
								}
								break;
							}
						}
...
INT Model::Tint_Explore_UpdateSpeed()
{
	...
	// ---- flip state
		vs_system::AppKamera *kamera = s_pi_model->app_kamera[s_pi_model->app_kamera_index];
		BOOL scalar_mode = kamera->GetScalarMode();

	// ---- increment mode
		scalar_mode = scalar_mode ? FALSE : TRUE;
		kamera->SetScalarMode(scalar_mode);

	// ---- use flag to stop rotation being multipled by ScalarIncr
		kamera->SetScalarRotation(TRUE);

	// ---- set translation and scalar (only used for user setting)
		kamera->SetScalarIncr(scalar_mode ? s_pi_model->kamera_scalar_increment : s_pi_model->kamera_scalar_increment/4.0f );
				...
				case '0':
					{
						...
						// ---- set camera update target text (e.g. "Update" + "about location of interest")
							Tint_Explore_UpdateElement(model_handle,
									Tint_Explore_Param::ELEMENT_LID_UPDATE_FIRST
								);
				...
				case 'I':
					{
						// ---- set speed element
							vs_system::AppKamera *kamera = s_pi_model->app_kamera[s_pi_model->app_kamera_index];
							BOOL scalar_speed = kamera->GetScalarMode();
							Tint_Explore_Switch(model_handle,
