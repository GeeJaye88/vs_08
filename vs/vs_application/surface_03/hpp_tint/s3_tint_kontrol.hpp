			...
			case TINT_HEAD_3DS_LID:
				{
					if ( handle_element != 0 )
					{
						...
						// ---- de/active analysis tabs
							model->TintHeadUpdateAnalysisTabButtons();

						// ---- visible objects
							model->TintBodyUpdateVisibleObjects(body_id);
	...
	// ---- get callbacks
		...
		switch ( group_id )
		{
			...
			case TINT_BODY_CALIBRATE_LID:
				model->Tint_Calibrate_GetCallbacks(&tint_callbacks);
				break;

			case TINT_BODY_LIGHT_LID:
			case TINT_BODY_LIGHT_PARAM_LID:
				model->Tint_Light_GetCallbacks(&tint_callbacks);
				break;
			...
			case TINT_BODY_ENHANCE_LID:
				model->Tint_Enhance_GetCallbacks(&tint_callbacks);
				break;
	...
	)
{
	...
	result = TintUpdateBody(app_kontrol, TINT_BODY_CALIBRATE_LID, 0);

	result = TintUpdateBody(app_kontrol, TINT_BODY_LIGHT_LID, 0);
	result = TintUpdateBody(app_kontrol, TINT_BODY_LIGHT_PARAM_LID, 0);
	...
	result = TintUpdateBody(app_kontrol, TINT_BODY_MEASURE_LID,   0);
	result = TintUpdateBody(app_kontrol, TINT_BODY_MEASURE_PARAM_LID, 0);

	result = TintUpdateBody(app_kontrol, TINT_BODY_ENHANCE_LID, 0);
	...
	result = TintUpdateBody(app_kontrol, TINT_BODY_SETTINGS_LID, 0);
