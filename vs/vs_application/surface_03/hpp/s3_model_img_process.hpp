...
INT Model::AcquireDemoCalculateLuminaFields()
{
	...
	// ---- acquire heightfield
		...
		note: Calculate acquisition demo source (y) heightfield array,
		      see other notes below concerning stride....
	...
	// ---- calc xyz normals ( stride = 3 )
		...
		result = vs_system::ObjConfig::Mesh_Normals( 
				y_array,
				y_array_width,
				norm_array,
				acquire_width,
	...
	// ---- calc surface xYz ( stride = 3 ) from y_array
		...
		*/
		...
		// -- geometry
			...
			FLOAT  y_min = 99999999.0f; // to big to fail!?
			FLOAT  y_max = -y_min;
		...
		for (UINT r = 0; r < atlas_page_width; r++)
		{
			...
			// -- get
				y_array_offset = r * atlas_page_width;
			// put
			...
			case vs_system::ObjConfig::MESH_VERSION_FUNCTION:
				{
					...
					// ---- height proportional to size of surface 
						FLOAT width = surface_03->GetAcquireSurfaceSize();
						height_scalar *= width;
