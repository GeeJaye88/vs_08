...
public:
	...
	enum
	{
		...
		// ---- gfx 1D list of object [ index, setup, display, callback, visible ] version 
			...
			GOB_EXP_LOI_ANNO_IDX  = 17,

			GOB_ACQ_LGT_RIG_IDX   = 18,
		...
		// ---- gfx element [ group, item ] handle list
			...
			ELM_EXP_LOI_ANNO_LIST_IDX = 11,

			ELM_ACQ_LGT_AXIS_LIST_IDX = 12,
	...
	// ---- gfx object list
		...
		{
			{ 0, ObjectExpSpot_Setup,     ObjectExpSpot_Display,    1,  0 },
			{ 1, ObjectExpSpot_Setup,     ObjectExpSpot_Display,    1,  0 },
			{ 2, ObjectExpSpot_Setup,     ObjectExpSpot_Display,    1,  0 },
			{ 3, ObjectExpSpot_Setup,     ObjectExpSpot_Display,    1,  0 },

			{ 0, ObjectExpLgtAxis_Setup,  ObjectExpLgtAxis_Display, 1,  0 },
			{ 1, ObjectExpLgtAxis_Setup,  ObjectExpLgtAxis_Display, 1,  0 },
			{ 2, ObjectExpLgtAxis_Setup,  ObjectExpLgtAxis_Display, 1,  0 },
			{ 3, ObjectExpLgtAxis_Setup,  ObjectExpLgtAxis_Display, 1,  0 },

			{ 0, ObjectSurfMesh_Setup,    ObjectSurfMesh_Display,   1,  0 },
			{ 0, ObjectSurfBase_Setup,    ObjectSurfBase_Display,   1,  0 },
			{ 0, ObjectSurfEdge_Setup,    ObjectSurfEdge_Display,   1,  0 },
			{ 0, ObjectSurfAxis_Setup,    ObjectSurfAxis_Display,   1,  0 },

			// -+- fix loi stuff -+-
			{ 0, ObjectLoiSpot_Setup,     ObjectLoiSpot_Display,    1,  0 },
			{ 0, ObjectLoiAxis_Setup,     ObjectLoiAxis_Display,    1,  0 },
			{ 0, ObjectLoiRect_Setup,     ObjectLoiRect_Display,    1,  0 },
			{ 0, ObjectLoiRule_Setup,     ObjectLoiRule_Display,    1,  0 },
			{ 0, ObjectLoiSect_Setup,     ObjectLoiSect_Display,    1,  0 },
			{ 0, ObjectLoiAnno_Setup,     ObjectLoiAnno_Display,    1,  0 },

			{ 0, ObjectAcqAxis_Setup,     ObjectAcqAxis_Display,    1,  0 },
		};
	...
	// -------- SURFACE CONFIGURATION properties -------- 
		...
		struct Surface_03_Config
		{
			...
			// ---- acquire dimensions proportional to size of surface
				//
				// note: currently the limit on size is due to the hardwired
				// # of vertices and lines in the vs_s3_[*] object methods
				//
				struct
				{
					FLOAT  limit    = 100.0f; // GetAcquireSurfaceLimit()
					FLOAT  size     = 20.0f; // GetAcquireSurfaceSize()
					FLOAT  width    = size;  
					FLOAT  height   = size;
					FLOAT  depth    = size;
				} acquire_dimensions;
			...
			// ---- target
				UINT      target_loi_index       = 0; // index >=0 <= MAX_EXP_LOI_
				UINT      target_loi_index_prev  = 0; // index >=0 <= MAX_EXP_LOI_
				BOOL      target_measure         = 0; // flag, TRUE if the Measure Tab is selected
				FLOAT     target_measure_fov     = 0; // Measure Tab vertical field of view
				BOOL      target_visible         = 0; // target spot & axis
				BOOL      target_ruler_visible   = 0; // straight rule and base cross
				BOOL      target_section_visible = 0; // dynamic bendy rule
				BOOL      target_profile_visible = 0; // dynamic bendy lower edge

				FLOAT     target_section_length  = 0; // between intersects
				FLOAT     target_section_height  = 0; // total maximum height
				FLOAT     target_section_max     = 0; // maximum value
				FLOAT     target_section_min     = 0; // minimum value

				FLOAT     target_loi_surface_y   = 0; 

				BOOL      target_target_mode     = 0; // msg "RFADWS" moves target or rotates target section
				BOOL      target_section_mode    = 0; // msg "RF" moves target along section/profile

				BOOL      target_init[MAX_EXP_LOI_];  // target object needs (re)initialising ?
				VsVector2 target_intersect[2];        // section surface intersection
...
INT Surface_03::PI_Surface_03::SetObjectsVisible()
{
	...
	// ---- all on
		...
		gfx_object_list[PI_Surface_03::GOB_EXP_LOI_ANNO_IDX].visible  = 1;
