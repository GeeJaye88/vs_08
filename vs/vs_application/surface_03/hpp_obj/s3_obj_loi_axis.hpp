	...
	// ---- elem
		for (int i = 0; i < PI_Surface_03::MAX_EXP_LOI_; i++)
		{
			...
			// ---- get target loi
				...
				param[vs_system::ObjConfig::BOX_FLAG_2] = p_s_03->GetAcquireHeightFieldMax();  // y_top tbd ?
				param[vs_system::ObjConfig::BOX_FLAG_3] = p_s_03->GetAcquireHeightFieldMin();  // y_bot
		...
		for (int i = 0; i < PI_Surface_03::MAX_EXP_LOI_; i++)
		{
			...
			// ---- visible ?
				...
				if ( elm_visible )
				{
					...
					// ---- not used - y object offset to "see" just above surface
						FLOAT y_off = 0;
					...
					// ---- transform
						...
						pimp->matrix_stack->Translate(v.x, v.y + y_off, v.z - 0.05f);
...
INT Surface_03::ObjectLoiAxis_Update(Surface_03 *p_s_03)
{
	...
	// ---- vb param
		...
		param[vs_system::ObjConfig::BOX_FACE_X] = v.x;
		param[vs_system::ObjConfig::BOX_FACE_Y] = v.y;
		param[vs_system::ObjConfig::BOX_FACE_Z] = v.z;
