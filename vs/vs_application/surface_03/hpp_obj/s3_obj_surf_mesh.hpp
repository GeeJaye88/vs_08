...
INT Surface_03::ObjectSurfMesh_UpdateParam(INT index, Surface_03 *p_s_03)
{
	...
	// ---- scalar to differentiate between bitmap & functional surfaces
		switch ( surface_mode )
		{
			case vs_system::ObjConfig::MESH_VERSION_FUNCTION:
				...
				break;
			default:
				param[vs_system::ObjConfig::MESH_SCALAR] = pimp->_configuration.aquire_height_field.scalar / 1;
				break;
		}


	// ---- note (important): proportional to surface size 
		param[vs_system::ObjConfig::MESH_SCALAR] *= p_s_03->GetAcquireSurfaceSize();
