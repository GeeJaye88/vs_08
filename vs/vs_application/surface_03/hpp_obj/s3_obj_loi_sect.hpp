			...
			// ---- visible ?
				...
				if ( elm_visible )
				{
					...
					// ---- transform
						...
						pimp->matrix_stack->Translate(0, v.y + y_off, 0);
...
INT Surface_03::ObjectLoiSect_Update(Surface_03 *p_s_03)
{
	...
	// ---- measure mode
		BOOL profile = p_s_03->GetTargetLoiProfileVisible();
	...
	// ---- store parameters
		...
		// ---- measuring ?
			param[vs_system::ObjConfig::BOX_FACE_MODE] = profile ? 1.0f : 0.0f;
		...
		// ---- at
			...
			param[vs_system::ObjConfig::BOX_FLAG_2] = y_ext + y_hgt; 
			param[vs_system::ObjConfig::BOX_FLAG_3] = y_ext;
		...
		// ---- marks_per_unit proportional to size of surface
			param[vs_system::ObjConfig::BOX_FLAG_8] = (FLOAT)((INT)( p_s_03->GetAcquireSurfaceLimit() / p_s_03->GetAcquireSurfaceSize() + 0.5f));
	...
	// ---- get section samples
		...
		hr = p_s_03->CalculateTargetLoi_Surface_Section(
				x3, y3,
				x4, y4,
