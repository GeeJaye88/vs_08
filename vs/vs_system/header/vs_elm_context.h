	...
	public:
		...
		// ---- element behaviours
			enum {
				...
				BEHAVIOUR_RADIO       = 4,   // RADIO all same group elements within a display list
				BEHAVIOUR_SWITCH      = 5,   // ON or OFF
				BEHAVIOUR_UNCLUNK     = 6,   // when set by application re-init to CLICK
				BEHAVIOUR_RADIO_LOCAL = 7,   // RADIO all LOCAL (contiguous) same group elements within a display list 
			};
		...
		// ---- element sub-behaviours
			enum {
				...
				SUB_BEHAVIOUR_RADIO_SWITCH = 2,   // overides RADIO & RADIO_LOCAL "only select if not selected"
			};


		// ---- element flags
			// note : 9/9/20 - NOT USED
			enum {
		...
		// -------- set
			...
			INT SetBitmask(UINT);
			INT SetBlinkEnd();
			INT SetBlinkFrame(UINT);
			INT SetBlinkMax(UINT);
