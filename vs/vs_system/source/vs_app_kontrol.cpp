	...
	)
{
	...
	// ---- update only if not second ONE - to catch spurious Release version panel callbacks...
		DOUBLE f = app_kontrol->pi_app_kontrol->sys_context->GetDouble(SysContext::TIME_MS_ELAPSED);
		if ( f < 1000 )
		{
			return VS_OK;
		}
	...
	// ---- IF NO interaction THEN return
		if ( handle_panel != NULL )
		{
			...
			result = app_view->GetPanelInteractionStatus(
						NULL,
						&interaction_status
					);
