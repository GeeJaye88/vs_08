...
\note See definition of VsFloatRectangle
*/
VsFloatRectangle *PanelContext::GetBorderRect()
{
...
// ---------- GetNumberOfDisplayLists ----------
/*!
\brief get number of display lists
\author Gareth Edwards
\param INT * - [in/out] handle
\return INT - ERROR <= 0 < VS_OK
*/
INT PanelContext::GetNumberOfDisplayLists(UINT *top)
{
...
\return BOOL - status
*/
BOOL PanelContext::GetBorder()
{
	return pi_panel_context->panel_border;
}
...
\return BOOL - status
*/
INT PanelContext::SetBorder(BOOL border)
{
	pi_panel_context->panel_border = border;
	return VS_OK;
}
	...
	// ---- data ?
		...
		while ( node != NULL )
		{
			...
			// ---- panel
				...
				{
					if (p->GetValue() != NULL)
						pimp->panel_border = p->GetBoolFromValue();
				}
