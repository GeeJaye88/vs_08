...
\note copied & ammended for VS from VSL_01 on 11/04/22
...
INT WinMessagePump()
{
	...
	// ---- frames per second
		...
		UINT fps_last = 0;
		UINT fps_now = 0;
		UINT fps = virtual_surface.sys_context->GetDWord(virtual_surface.sys_context->FPS);
						...
						if (d_ms_elapsed > d_ms_elapsed_target)
						{
							...
							// ---- invoke display method
								...
								try
								{
									...
									// ---- display
										INT error_code = virtual_surface.app_interface.Framework(
									...
									// ---- update seconds & reset fps if next second
										if (seconds_elapsed != seconds_elapsed_last)
										{
											...
											virtual_surface.sys_context->SetDWord(virtual_surface.sys_context->FPS_NOW, (DWORD)fps_now);
							...
							// ---- catch, report and return unexpected error
								catch ( INT error_id )
								{
									...
									sprintf_s(
											...
											"    id %d, and \n"
											"    HRESULT %d.",
											error_id, hr // in vsl was SUCCESS, etc..
										);
						...
						if ( time_elapsed_ms > next_time )
						{
							...
							// ---- TRY to invoke display method
								...
								try
								{
									...
									// ---- regulate
										DOUBLE time_to_display = (DOUBLE)timeGetTime() - start_time;
									...
									// ---- fps NOW
										...
										{
											virtual_surface.sys_context->SetDWord(virtual_surface.sys_context->FPS_NOW, (DWORD)fps_now);
											virtual_surface.gfx_interface->SetFpsNow(fps_now);
											//CHAR ods[128];
											//sprintf_s(ods, 128, "WinPump: next second after %d - %f %f\n", fps_now, (float)time_elapsed_ms, (float)time_elapsed_ms_last);
...
INT WinCreate (HINSTANCE hInstance)
{
	...
	try
	{
		...
		// ---- set
			...
			HRGN  hRgn = CreateRectRgn(border, border, width-border, height-border);
