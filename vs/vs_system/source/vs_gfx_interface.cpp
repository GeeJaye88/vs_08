...
// ---------- RenderViewrect ----------
/*!
\brief Render Viewrect background and foreground (e.g. outline, grid)
\author Gareth Edwards
\return INT - ERROR # < VS_ERROR < VS_OK
*/
	...
	)
{
	...
	// ---- display list
		UINT display_list_count = 0;
			panel_context->GetNumberOfDisplayLists(&display_list_count);

		UINT dspl_list_top = display_list_count - 1;

		UINT dspl_list_index = 0;   
			panel_context->GetDisplayListIndex(&dspl_list_index);

		BOOL dspl_list_first = dspl_list_index == 0 ? TRUE : FALSE;
		BOOL dspl_list_last  = dspl_list_index == dspl_list_top ? TRUE : FALSE;


	// ---- background ?
		//
		// note: gates both background and outline
		//
		BOOL background = panel_context->GetBackground();
		BOOL border = panel_context->GetBorder();
	...
	// ---- render border ?
		if ( render_border )
		{
			...
			// ---- last in display list ?
				if ( dspl_list_last && background )
				{
		...
		else
		{
			...
			// ---- first in display list ?
				if ( dspl_list_first && background )
				{
	...
	)
{
	...
	// ---- display ok?
		...
		/*
		ElmContext *elm_context = &display_list->element_context;

		CHAR name[28];
		strcpy(name, display_list->element_context.GetName().c_str());
		if ( strcmp(name, "DotObj") != 0 )
		// if ( ! elm_context->GetDisplay() )
		{
			return VS_OK;
		}
		*/
	...
	// ---- NOT picked element - SEE 5.4 pick panel
		BOOL picked_an_element = FALSE;


	// ---- Frames Per Second NOW! (used to blink)
		UINT fps_now = pi_props->fps_now;
			...
			if ( element->element_context.GetVisible() && element->element_context.GetDisplay() )
			{
						...
						// ---- apply actions [index/id, value], [], ...
							...
							for ( UINT action = 0; action < num_actions; action++ )
							{
								...
								switch ( element_previously_contex->GetBehaviour() )
								{
									...
									case ElmContext::BEHAVIOUR_RADIO_LOCAL:
														...
														case ElmContext::BEHAVIOUR_SWITCH:
															{
																if ( action_value == 1 )
																{
																	e->element_context.SetMultiActive(TRUE, 0);
																}
																else
																{
																	e->element_context.SetMultiActive(FALSE, 0);
																}
															}
															break;
														default:
				...
				// 5.2.2.2 setup 3D lighting ===================================================
					...
					if ( lit )
					{
						...
						// ---- ambient
							BOOL ambient = element_context->GetAmbient();
							if ( ambient &&  pi_props->gfx_ambient->ambient_enable )
							{
								D3DCOLORVALUE *c = &pi_props->gfx_ambient->color;
								hr = SetAmbient(c->r, c->g, c->b);
							}
							else
							{
								hr = SetAmbient(0,0,0);
					...
					// ---- fog
						hr = SetFogEnabled(element_context->GetFog());
					...
					if ( element->over && behaviour > ElmContext::BEHAVIOUR_NONE )
					{
						...
						// ---- picked ?
							...
							if (
									...
									! picked_an_element // ---- NOT picked element - SEE 5.4 pick panel
								)// 
							{

								// ---- really important tidy up ---- !!!
									//
									//   if behaviour == RADIO/RADIO_LOCAL/CLICK then cancel pick
									...
									switch ( behaviour )
									{
										...
										case ElmContext::BEHAVIOUR_CLICK:
											...
											{
												element->picked = FALSE;
												element_context->SetBlinkEnd();
											}
											else
											{
												element_context->SetBlinkFrame(0);
												element_context->SetBlinkMax(fps_now/4);
											}
											break;
										default:
											break;
								...
								// ---- NOT picked element- BUT element had been ALREADY picked... "HERE" & SEE 5.4 pick panel
									if ( ! element->picked )
				...
				// ---- already ON ?
					...
					// ON THE TIN....
					//BOOL report = FALSE;
					//if (element_context->GetGroupId() == 35 && element_context->GetId() == 11)
					//{
						//if (
						//	element->picked || picked_an_element &&
						//	switched_on_status &&
						//	element_context->GetVertexCount() == 4
						//	)
						//{
						//	report = TRUE;
						//	char msg[16];
						//	sprintf(msg, " T -> Id %d %d\n", element_context->GetId(), (INT)switched_on_status);
				...
				// 5.2.2.10 update on behaviour ================================================
					...
					switch ( behaviour )
					{
						...
						case ElmContext::BEHAVIOUR_RADIO:
							...
							if ( element->picked ) 
							{
								if ( switched_on_status )
								{
									...
									// can't have a radio button with by itself....
									if ( counter == 0 )
									{
										switched_on_status = FALSE;
										element_context->SetSwitchedOn(switched_on_status);
								...
								else
								{
									...
									while ( e )
									{
										...
										if ( e_context->GetGroupId() == group_id )
										{
											...
											//	if (e_context->GetBehaviour() == ElmContext::BEHAVIOUR_RADIO)
											//	{
											//		e->element_context.SetSwitchedOn(TRUE);
											//	}
											//}
										}
										e = e->next;
						...
						// RADIO a contiguous local group within a display list
						case ElmContext::BEHAVIOUR_RADIO_LOCAL:
								...
								if ( element->picked )
								{
									...
									// ---- already switched on?
										if ( switched_on_status )
										{
											...
											// ---- single switch, if:
											//      - number of other radio buttons == 0
											//      - treat as a switch
											...
											// ---- count
												...
												while ( e )
												{
													...
													if ( e_context->GetGroupId() == group_id )
													{
														if ( e_context->GetId() != id )
														{
															if ( e_context->GetBehaviour() == ElmContext::BEHAVIOUR_RADIO_LOCAL )
															{
																counter++;
															}
														}
										...
										else
										{
											...
											// ---- find first in group
												PI_Element *el = element->last;
												while ( el )
												{
													vs_system::ElmContext *el_context = &el->element_context;
													if ( el_context->GetGroupId() == group_id )
													{
														if ( el_context->GetBehaviour() == ElmContext::BEHAVIOUR_RADIO_LOCAL )
														{
											...
											// ---- switch all but this off
												PI_Element *en = element->next;
												while  ( en )
												{
													vs_system::ElmContext *en_context = &en->element_context;
													if ( en_context->GetGroupId() == group_id )
													{
														if ( en_context->GetBehaviour() == ElmContext::BEHAVIOUR_RADIO_LOCAL )
														{
					...
					// ---- multi-state texturing ?
						...
						if ( element_context->GetMultiState() )
						{
							...
							// ---- multi state ?
								...
								UINT blink_max   = element_context->GetBlinkMax();
								UINT blink_frame = element_context->GetBlinkFrame();
								BOOL blink_on_status = blink_frame >= 0 && blink_frame < blink_max;
								if ( blink_on_status )
									element_context->SetBlinkFrame(blink_frame+1);

								texture_page_reference =
									element_context->GetMultiActive() ?
										(
											switched_on_status || blink_on_status ? pr[2] : ( element->over ? pr[3] : pr[1] )
										) : pr[element_context->GetMultiNull()];

								//if (report)
								//{
								//	char msg[64];
								//	sprintf(msg, "    > Id %d - TPR %d - SOS %d - BOS - %d - EO %d\n",
								//		element_context->GetId(),
								//		texture_page_reference,
								//		(INT)switched_on_status,
								//		(INT)blink_on_status,
								//		(INT)element->over
								//	);
								//	OutputDebugString(msg);
					...
					if ( d3d9_texture != NULL )
					{
						...
						// ---- texture AND alpha blended
							if ( element_context->GetAlphaBlending() )
							{
								...
								BOOL alpha_texture = element_context->GetAlphaTexture();

								if (alpha_texture)
								{
									// ---- colour to come completely from the texture
										hr = d3d9_device->SetTextureStageState(0,D3DTSS_COLOROP,   D3DTOP_SELECTARG1);
										hr = d3d9_device->SetTextureStageState(0,D3DTSS_COLORARG1, D3DTA_DIFFUSE);
										hr = d3d9_device->SetTextureStageState(0,D3DTSS_COLORARG2, D3DTA_TEXTURE);   //Ignored

									// ---- alpha to come completely from the diffuse
										...
										hr = d3d9_device->SetTextureStageState(0, D3DTSS_ALPHAARG2, D3DTA_DIFFUSE);   //Ignored
								}
								else
								{
									...
									// ---- set colour and alpha to come completely from the texture
										hr = d3d9_device->SetTextureStageState(0,D3DTSS_COLOROP,   D3DTOP_SELECTARG1);
				...
				if ( element_on )
				{
					...
					// 5.2.2.16.4 render primitive =================================================
						...
						// TEMP
						//
						//if (element_context->GetText() != "")
						//{
						//	char txt[128];
						//	strcpy_s(txt, 128, element_context->GetText().c_str());
						//	if (!strcmp(txt, "# Keypad"))
						//	{
	...
	// ---- does this adapter mode support a 24 or 16bit depth stencil pixel format, ...?
		...
		{		
			...
			// ---- check for hardware T&L
				D3DCAPS9 m_caps;	
				g_pD3D->GetDeviceCaps(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, &m_caps);
...
INT GfxInterface::SetupGraphics()
{
	...
	// ---- dx fonts
		...
		INT setup_dx_fonts = SetupDxFonts();
		#endif
...
INT GfxInterface::CleanupGraphics()
{
	...
	// ---- dx fonts
		#if defined(CD3DXFONT)
		INT cleanup_dx_fonts = CleanupDxFonts();
		#endif
...
INT GfxInterface::SetupDxFonts()
{
		...
		for (INT i = 0; i < 8; i++)
		{
			...
			// ---- instantiate a font object
				pi_props->dx_font[i] = new CD3DFont(
						pi_props->dx_font_type[i],
						pi_props->dx_font_size[i],
						0
					);

			// ---- initialise device-dependent objects, including the
				// vertex buffer used for rendering text and the texture
				// map which stores the font image.
				//
				pi_props->dx_font[i]->InitDeviceObjects(pi_props->gfx_device->d3d9_device);

			// ---- create vertex buffer for the letters, reserve state
				// blocks and set ther appropriate render states
				pi_props->dx_font[i]->RestoreDeviceObjects();
...
INT GfxInterface::CleanupDxFonts()
{
		...
		for (INT i = 0; i < 8; i++)
		{
			delete pi_props->dx_font[i];
			pi_props->dx_font[i] = NULL;
		}
...
CD3DFont * GfxInterface::GetDxFont(INT index)
{

	#if defined(CD3DXFONT)
	return pi_props->dx_font[index];
	#else
	return VS_ERROR;
	#endif
...
INT GfxInterface::SetDxFont(INT index, CHAR *type, INT size)
{

	#if defined(CD3DXFONT)
	INT i = index < 0 ? 0 : (index + 1 > 8 ? 7 : index);
	strcpy_s(pi_props->dx_font_type[i], VS_MAXCHARNAME, type);
	pi_props->dx_font_size[i] = size;
	return VS_OK;
	#else
	return VS_ERROR;
	#endif
