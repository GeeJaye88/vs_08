...
// ---------- vs_pano_cylinder_top_ring.hpp ----------
/*!
\file vs_pano_cylinder_top_ring.hpp
\brief
...
INT ObjConfig::Callback_PanoCylinderTopRing_Configure(vs_system::ElmContext *context)
{
	...
	if (!context->GetOveride())
	{
		...
		context->SetCullMode(ElmContext::CULL_NONE);
	}

	// ---- index buffer ----
	context->SetIndexCount(rows * cols * 6);
	context->SetIndexBufferCallback(&Callback_PanoCylinderTopRing_GetIndexBuffer);
	...
	context->SetVertexBufferCallback(&Callback_PanoCylinderTopRing_GetVertexBuffer);
...
// ----------  Callback_PanoCylinderTopRing_GetVertexBuffer ----------
/*!

*/
INT ObjConfig::Callback_PanoCylinderTopRing_GetVertexBuffer(vs_system::ElmContext *context)
{
	...
	for (UINT row = 0; row<vertex_rows; row++)
	{
		...
		// 3D 
		FLOAT y = x_axis_scalar * height + hby2 - incr_vertex_rows;
		...
		for (UINT col = 0; col<vertex_cols; col++)
		{
			...
			*(f + 2) = -z * cosf(y_rotation_radians);
			...
			if (vertex_format & VS_TEX1)
			{
				...
				*(f) = v * sinf(a) * aspect_ratio + u_centre;
				*(f + 1) = v * cosf(a) + v_centre;
				f += 2;
			}
		}
	}
...
// ---------- Callback_PanoCylinderTopRing_GetIndexBuffer ----------
INT ObjConfig::Callback_PanoCylinderTopRing_GetIndexBuffer(vs_system::ElmContext *context)
{
