...
public:
	...
	// ---- auto vptf (9 panels + 1 for auto loop)
		enum {
			MAX_AUTO_VPTF = 10
		};
	...
	// ---- auto
		BOOL IsAutoPTFRunning();
		BOOL IsAutoPTFStart();
		BOOL IsAutoPTFStop();
		BOOL IsAutoPTFSosi();
		BOOL IsAutoPTFSlide();
		BOOL IsAutoPTFSlow();
		BOOL IsAutoPTFForward();
		BOOL IsAutoPTFPause();
		BOOL IsAutoPTFShuffle();

		INT  GetAutoPTFFromId(UINT *from_id, FLOAT *from_mu);

		INT  SetAutoPTFRunning(BOOL running);
		INT  SetAutoPTFScalar(FLOAT ms_scalar);
		INT  SetAutoPTFSosi(BOOL sosi);
		INT  SetAutoPTFSlide(BOOL slide);
		INT  SetAutoPTFSlow(BOOL slow);
		INT  SetAutoPTFForward(BOOL forward);
		INT  SetAutoPTFPause(BOOL pause);
		INT  SetAutoPTFShuffle(BOOL shuffle);

		INT  AutoPTFStart();
		INT  AutoPTFSnapshot();
		INT  AutoPTFSwitchedOn(BOOL switched_on[]);
		INT  AutoPTFUpdate();
		INT  AutoPTFStop();
		INT  AutoPTFSet();
	...
	// ---- get ptf
		...
		INT GetPanoAutoPan(INT *); // not used
	...
	// ---- surface axis - note: returns max dimension
		static INT SurfAxis_GetDpuMajorMinor( 
				INT width,
				INT height,
				INT depth,
				INT *dpu,
	...
	// ---- loi annotation
		static INT Callback_S3_Loi_Annotation_Configure(vs_system::ElmContext*);
		static INT Callback_S3_Loi_Annotation_GetVertexBuffer(vs_system::ElmContext*);
		static INT Callback_S3_Loi_Annotation_GetIndexBuffer(vs_system::ElmContext*);
	...
	// ---- Lines 3D
		/*
		static INT ObjConfig::Lines3D_GetAxis(
		...
		*/
		...
		static INT ObjConfig::Lines3D_GetAsciiText(
				UCHAR *text,    // [in] text (e.g. "105.32")
		...
		static FLOAT ObjConfig::Lines3D_GetAsciiTextWidth(
				UCHAR *text,	// [in] text (e.g. "105.32")
				FLOAT  width,	// [in] char width
				FLOAT  space	// [in[ char spacing
			);

		static INT ObjConfig::Lines3D_GetAsciiChar(
				UCHAR *uch,		// [in] ASCII char from 0 to 255
				CHAR   align,   // [in] axis alignment
				FLOAT  colour,  // [in] colour
				FLOAT  width,	// [in] char width
				FLOAT  height,  // [in] char height
		...
		static FLOAT ObjConfig::Lines3D_GetAsciiCharWidth(
				UCHAR uch		// [in] ASCII char from 0 to 255
			);
