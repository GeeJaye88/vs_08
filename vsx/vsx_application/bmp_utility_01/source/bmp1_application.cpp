...
// ---- include
#include "../header/bmp1_application.h"


// ---- namespace
using namespace bmp_utility_01;
		...
		)
{
	...
	// ---- development
		...
		#ifdef TEST
			argc = 7;
			strcpy(argv[0], "bmp_utility");
			strcpy(argv[1], "-i");          // -i(n)  [CHAR] input filename
			strcpy(argv[2], "input.bmp");
			strcpy(argv[3], "-o");          // -o(ut) [CHAR] output filename.
			strcpy(argv[4], "output.bmp");
			strcpy(argv[5], "-b");          // -b(pp) [#] output bits per pixel
			strcpy(argv[6], "8");
			strcpy(argv[5], "-t");          // -t(id) [#] transform id
			strcpy(argv[6], "0");
		#else
			...
			while ( arg < argc )
			{
				...
				else
				{
					if ( arg + 1 < argc )
					{
						...
						// ---- version
							if      ( !strcmp("-i", argv[arg]) ) { name  = argv[arg + 1]; arg += 2; }  // -i(n)  [CHAR] input filename
							else if ( !strcmp("-o", argv[arg]) ) { name  = argv[arg + 1]; arg += 2; }  // -o(ut) [CHAR] output filename
							else if ( !strcmp("-b", argv[arg]) ) { major = argv[arg + 1]; arg += 2; }  // -b(pp) [#] output bits per pixel
							else if ( !strcmp("-t", argv[arg]) ) { minor = argv[arg + 1]; arg += 2; }  // -t(id) [#] transform id
...
INT application::PI_Application::Usage(CHAR mode)
{
	...
	printf("Usage: bmp_utility [output filename] [...]\n");
	printf("\n");

	printf("Arguments: \"bmp_utility\" OR \"bmp_utility.exe\"\n");
	printf("           followed by: [output filename (e.g., fred], then \n");
	printf("                        [-c(ommand) value] pairs \n");
	printf("\n");

	printf("Command options:\n");
	printf("\n");
	printf("         -i(n)  [CHAR] input filename.\n");
	printf("         -o(ut) [CHAR] output filename.\n");
	printf("         -b(pp) [#] output bits per pixel.\n");
	printf("         -t(id) [#] transform id.\n");
	printf("\n");
